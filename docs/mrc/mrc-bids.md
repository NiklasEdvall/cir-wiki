...
Note that the ASL sequence (Series number 5) is split in two entries. Some sequences are split and need to be BIDSified separately. In the case of this ASL sequence, we see in the column `Image type` that the first entry is the `ORIGINAL` ASL data and the second entry the `DERIVED` m0 reference scan.
...
- Intended for: Is used to specify the intended use of for example a fieldmap sequence, e.g. `fieldmap` or `sbref` for BOLD reference scans. This info is not included in the file name but is included in the JSON sidecar file and can be used by pre-processing pipelines to automatically identify fieldmaps and reference scans for their intended use.
...
This protocol include 4 `fmap` sequences, intended to be used as reference either for `fmri` or `dwi` sequences, and this is specified with the `Intended for` column. The `desc` column is used to distinguish two epi fieldmaps as the `Intended for` column is **not** included in the file name. Subsequently, these files will be named `sub-012_ses-01_desc-dwi_epi.nii.gz` and `sub-012_ses-01_desc-fmri_epi.nii.gz` in the BIDS output. Without the `desc` field these would both collide on the file name `sub-012_ses-01_epi.nii.gz` in this case.
